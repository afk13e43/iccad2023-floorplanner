...
int main(int argc, char *argv[])
{
    ...
    {
        while(!cases[i].empty() && cases[i].back() == '\n')
        {
            cases[i].pop_back();
        }
        //cout << "case " << i + 1 << ": " << endl;
    ...
    {   
        ...
        double accept=log(0.955);
        double accept2=log(0.95);
...
void GetCost_leftdown2(int & cost , vector<int>place_order,vector<int>&cantpush2,int first_change_module,int change_stop_module,vector<int>new_place_direction,vector<Club>INClub)//rat>0.9
{
 // cout<<"IN left down"<<endl;
    ...
    ++datacount;
    vector<int>overlap_soft=place_order;
    std::unordered_set<int>has_module;
    vector<int> cantpush;
    int bx,by;
    bool space= false;
    const int Msize = Modules.size();
        ...
        if(new_place_direction[p]==0)
        {
            ...
            if(p==INClub[club].Club_member[0])//club的頭
            {
                ...
                if(it == overlap_soft.begin())
                {
                    ...
                    for(int p =0;p<left_up_point.size();++p)
                    {
                        double distance= sqrt((left_up_point[p].getX()*left_up_point[p].getX())+(left_up_point[p].getY()*left_up_point[p].getY()));
                        if(min_distance>distance)
                        {
                            min_distance=distance;
                            min_point=left_up_point[p];
                        }
                    }
        ...
        else
        {
            ...
            if(p==INClub[club].Club_member[0])//club的頭
            {
                ...
                if(it == overlap_soft.begin())
                {
                    ...
                    double min_distance=DBL_MAX;
                    leftdown_corner min_point;
                    for(int p =0;p<right_down_point.size();++p)
                    {
                        double distance= sqrt((right_down_point[p].getX()*right_down_point[p].getX())+(right_down_point[p].getY()*right_down_point[p].getY()));
                        if(min_distance>distance)
                        {
                            min_distance=distance;
                            min_point=right_down_point[p];
                        }
                    }
                    int maxby=0;
    ...
    else if(event>=0.2&&event<=0.6)//event>0.45&&event<=0.9
    {
        ...
        else
        {
            if(event2<0.5)//組外換順序
            {
                ...
                unordered_set<int>has_connect_with_choosed;
...
void replace_soft_to_better_space(int bestway,vector<int>small_first)
{
    ...
    for(int t =0;t<tempsmall_first.size();++t)
    {
        ...
        //cout<<"i :"<<i<<endl;
        map<int, Module> temp_Modules=Modules;
        for(double ratio =0.5 ;ratio<=2.0;ratio=ratio+0.1)
        {
            be_changeAspectRatio(Modules[i],ratio);
            if(bestway==0)//左下
            {
                left_down_replace(temp_Modules, i);
            }
            else if(bestway==1)//左上
            {
                left_up_replace(temp_Modules,i);
            }
            else if(bestway==2)//右下
            {
                right_down_replace(temp_Modules,i);
            }
            else if(bestway==3)//右上
            {
                right_up_replace(temp_Modules,i);
            }
            Modules=temp_Modules;
        };
    };
...
void merge()
{
    ...
    for(vector<pair<pair<int, int>, int>>::iterator it=keyValueVec.begin();it!=keyValueVec.end();++it)
    {
        ...
        if(hasdoMerge.find(mod.first)==hasdoMerge.end()&&hasdoMerge.find(mod.second)==hasdoMerge.end())
        {
            ...
            cout << "end: (" << end.x << ", " << end.y << ")" << ", by connectPIN: "<<value<<endl;
            //做融合
